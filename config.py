...
# 상태-행동 공간 설정
STATE_DIM = (
    FEATURE_DIM + 1 + len(SYMBOLS)
)  # 상태 차원: 시장특성(12) + 위기레벨(1) + 이전가중치(30) = 43
ACTION_DIM = len(SYMBOLS)  # 행동 차원: 포트폴리오 가중치(30)
HIDDEN_DIM = 128  # 신경망 은닉층 차원

# SAC 하이퍼파라미터 (agents/bcell.py에서 사용) - 안정성 우선 v5.2
LR_ACTOR = float(1e-4)  # Actor 네트워크 학습률 (압축본 권장값)
LR_CRITIC = float(1e-4)  # Critic 네트워크 학습률 (압축본 권장값)
WEIGHT_DECAY = float(1e-6)  # 가중치 감쇠 (L2 정규화, 약한 정규화 추가)
GAMMA = float(0.99)  # 할인율 (표준값으로 안정화)
TAU = float(0.005)  # 타겟 네트워크 EMA 업데이트 비율 (보수적 설정)
BATCH_SIZE = int(512)  # 미니배치 크기 (압축본 권장 - 더 안정적 그래디언트)
BUFFER_SIZE = int(20000)  # 경험 재생 버퍼 크기 (다양성 증대)
REPLAY_CAPACITY = int(100000)  # 압축본 권장 - 확장된 버퍼 크기
REWARD_SCALE = float(0.3)  # Q 폭주 방지용 보상 스케일링 (압축본 권장 - 보수적)
UPDATE_FREQUENCY = int(1)  # 네트워크 업데이트 주기 (압축본 권장)

# SAC 온도 파라미터 (temperature α - 스칼라) - 안정성 우선
INIT_ALPHA_TEMP = float(0.05)  # SAC temperature 초기값 (보수적 탐험)
TARGET_ENTROPY_PER_DIM = float(-0.5)  # SAC 목표 엔트로피 (압축본 권장 - 완화)
TARGET_ENTROPY_MIN_PER_DIM = float(-0.5)  # 스케줄 미사용 시 base와 동일
TARGET_ENTROPY_MAX_PER_DIM = float(-0.5)  # 스케줄 미사용 시 base와 동일
TARGET_ENTROPY_SCHEDULE = "none"  # 스케줄 비활성화 ("none", "linear", "cosine")
