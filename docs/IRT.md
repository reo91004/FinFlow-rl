...
│                                                                            │
│  ┌──────────────────────┐        sample mini-batch        ┌────────────────┐│
│  │     Replay Buffer    │────────────────────────────────►│   Actor π_θ    ││
│  │    (s,a,r,s',d)      │◄────────────────────────────────│   (IRT)        ││
│  └──────────────────────┘        store transitions        │  BCellIRTActor ││
│                                                           │                ││
│                                                           │  ┌──────────┐  ││
│                                                           │  │T-Cell &  │  ││
│                                                           │  │Signal Det│  ││
...
│                                                            └────────────────┘│
│                                                                            │
│  ┌──────────────────────┐    TD targets / gradients    ▲                    │
│  │ Twin Critics Q_ϕ1,ϕ2 │◄────────────────────────────┘                    │
│  │     (value nets)     │───────── sample (s,a) from Replay Buffer         │
│  └──────────────────────┘                                                  │
│             ▲                                                              │
│             │ Q(s, π_j(s)) fitness                                         │
│             └────────────────────────────┐                                  │
...
- c=0 (완전 평시) → α_c=0.45 (OT 지배, 균등 분산)
- c=0.5 (경계) → α_c≈0.27 (균형)
- c=1 (완전 위기) → α_c=0.08 (Replicator 지배, 선택적 집중)

**직관**:

- 평시: 균등 분산 (OT가 모든 프로토타입 고려)
- 위기: fitness 기반 집중 (Replicator가 Q-value 높은 전문가 선택)

**주의**: α↓는 "보수적"이 아니라 "집중"을 의미합니다. Replicator는 Q-value가 높은 전문가를 선택하므로, 이것이 방어적인지 공격적인지는 학습된 Q-function에 달려있습니다.

### Sharpe Gradient Feedback (Phase C)

**⚠️ 중요**: 현재 Sharpe 피드백 설계에는 reward 함수와의 충돌이 존재합니다.

Sharpe ratio 변화율을 α_c에 피드백하여 성능 추세 반영:

```
Δsharpe = DSR_bonus  # 입력 신호 (원본 스케일 ~0.1)
α_c' = α_c · (1 + 0.25·tanh(Δsharpe)) + 0.0·tanh(Δsharpe)
α_c_final = clamp(α_c', α_min, α_max)
```

**현재 파라미터** (코드베이스 기준):

- `alpha_feedback_gain = 0.25` (곱셈 항)
- `alpha_feedback_bias = 0.0` (가법 항, 제거됨)
...
1. α(c)↓ (위기 시): Replicator 활성 = 선택적 집중
2. κ(c)↑ (위기 시): Sharpe bonus 증가 = 고수익 추구

→ 정책이 상충된 신호를 받아 α가 좁은 범위에 고착됩니다.
...
- α↓: Replicator 지배 = Q-value 높은 전문가에게 집중
- 이것이 방어적인지 공격적인지는 **학습된 Q-function에 달려있음**
- Replicator는 단지 "과거 성공 전략"을 선택할 뿐

**올바른 해석**:

- α↑ (OT 지배): 균등 분산 (모든 프로토타입 고려)
- α↓ (Replicator 지배): 선택적 집중 (fitness 기반 차별)

### Reward 함수와의 일관성

**핵심 원칙**: T-Cell (위기 감지)과 Reward (행동 유도)는 **같은 방향**을 가리켜야 합니다.

**현재 문제**:

```
T-Cell:  위기 → α↓ → Replicator 활성 → 집중
Reward:  위기 → κ↑ → Sharpe 추구 → 고수익
...
**비유는 직관을 돕지만 정확하지 않습니다**:

| 생물학적 면역                   | IRT 실제                   | 차이점                 |
| ------------------------------- | -------------------------- | ---------------------- |
| T-Cell 활성화 → 위기 대응       | T-Cell → crisis_level 출력 | T-Cell은 **센서**일 뿐 |
| B-Cell 항체 생산 → 방어         | B-Cell → 포트폴리오 생성   | "방어"는 학습된 결과   |
| 위기 시 특정 항체 집중 → 보수적 | 위기 시 α↓ → 집중          | 집중≠보수적            |

**비유를 넘어서**:

- IRT는 **강화학습** 프레임워크입니다
- Q-function이 위기 시 방어 전략에 높은 Q-value를 할당하도록 **학습되어야** 비로소 "위기 대응"이 가능합니다
- Reward 설계가 이를 유도해야 합니다 (κ(c) 역전)

---

**문의**: GitHub Issues 또는 Discussions 활용

**마지막 업데이트**: 2025-10-14 (논리적 오류 수정 반영)
