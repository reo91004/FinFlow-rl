...
def integrated_gradients(model, state, baseline=None, steps=50):
    ...
    for alpha in alphas:
        ...
        # 평탄화된 상태에 대해 모델 실행
        # 수정: actor_critic_base를 직접 호출
        action_probs = model.actor_critic_base(interpolated_tensor)
...
def compute_feature_weights_drl(ppo_agent, states):
    ...
    for state in tqdm(states):
        # 통합 그래디언트 계산 - 수정: ppo_agent.policy를 직접 전달
...
def plot_performance(portfolio_values, benchmark_values=None, title="Portfolio Performance", filename=None):
    """
    포트폴리오 성능 시각화 및 파일 저장
    """
    ...
    # 파일명이 지정되지 않은 경우 현재 시간을 사용
    if filename is None:
        current_time = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f'portfolio_performance_{current_time}.png'
...
def plot_feature_importance(drl_weights, reference_weights, feature_names, filename=None):
    """
    DRL 에이전트와 참조 모델의 특성 중요도 비교 시각화
    """
    # 입력 데이터 검증
    if len(drl_weights) == 0 or len(reference_weights) == 0:
        print("경고: 가중치 배열이 비어있습니다.")
        return
        
    if len(drl_weights) != len(feature_names) or len(reference_weights) != len(feature_names):
        print(f"경고: 가중치 배열의 길이({len(drl_weights)}, {len(reference_weights)})가 특성 이름의 개수({len(feature_names)})와 일치하지 않습니다.")
        return
    ...
    bars = plt.bar(x, drl_weights)
    plt.xticks(x, feature_names, rotation=45, ha='right')
    plt.title('DRL Agent Feature Importance')
    ...
    # 참조 모델의 특성 중요도
    plt.subplot(1, 2, 2)
    bars = plt.bar(x, reference_weights)
    plt.xticks(x, feature_names, rotation=45, ha='right')
    plt.title('Reference Model Feature Importance')
    plt.ylabel('Importance Score')
    ...
    if filename is None:
        current_time = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f'feature_importance_{current_time}.png'
    ...
    print(f"특성 중요도 그래프가 저장되었습니다: {save_path}")

def plot_correlation_analysis(correlations, filename=None):
    """
    DRL 에이전트와 참조 모델 간의 상관관계 분석 시각화
    """
    plt.figure(figsize=(12, 6))
    plt.plot(correlations, marker='o', linestyle='-', linewidth=1, markersize=5)
    plt.axhline(y=0, color='r', linestyle='--')
    plt.title('Correlation between DRL and Reference Model')
    ...
    plt.axhline(y=mean_corr, color='g', linestyle='--', label=f'Mean: {mean_corr:.2f}')
    plt.legend()
    
    # 파일명이 지정되지 않은 경우 현재 시간을 사용
    if filename is None:
        current_time = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f'correlation_analysis_{current_time}.png'
    ...
    print(f"상관관계 분석 그래프가 저장되었습니다: {save_path}")

def plot_integrated_gradients(ig_values, feature_names, filename=None):
    """
    통합 그래디언트 결과 시각화
    """
    plt.figure(figsize=(12, 6))
    x = np.arange(len(feature_names))
    bars = plt.bar(x, ig_values)
    plt.xticks(x, feature_names, rotation=45, ha='right')
    plt.title('Integrated Gradients Analysis')
    ...
    if filename is None:
        current_time = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = f'integrated_gradients_{current_time}.png'
    ...
    print(f"통합 그래디언트 분석 그래프가 저장되었습니다: {save_path}")
...
def main():
    ...
    if test_results is None:
         ...
         plot_performance(
             ...
             title="PPO Portfolio Performance (Evaluation)",
             filename=f'portfolio_performance_{current_time}.png'
         )
         ...
         try:
             # 데이터 길이 확인 및 조정
             print("데이터 길이 확인 중...")
             min_length = min(len(test_data), len(test_results['returns']))
             print(f"사용할 데이터 길이: {min_length}")
             
             # 데이터 길이 맞추기
             test_data_aligned = test_data[:min_length]
             returns_aligned = test_results['returns'][:min_length]
             
             # DRL 에이전트의 특성 가중치 계산
             print("DRL 에이전트의 특성 가중치 계산 중...")
             drl_weights = compute_feature_weights_drl(ppo_agent, test_data_aligned)
             if drl_weights is None or len(drl_weights) == 0:
                 print("경고: DRL 가중치 계산 실패")
                 return
                 
             print("참조 모델의 특성 가중치 계산 중...")
             reference_weights = linear_model_hindsight(test_data_aligned, returns_aligned)
             if reference_weights is None or len(reference_weights) == 0:
                 print("경고: 참조 모델 가중치 계산 실패")
                 return
                 
             # 가중치의 차원 확인 및 조정
             if drl_weights.ndim > 1:
                 drl_weights = drl_weights.mean(axis=0)  # 시간에 따른 평균
             if reference_weights.ndim > 1:
                 reference_weights = reference_weights.mean(axis=0)
                 
             print("특성 중요도 비교 시각화 중...")
             ...
             print("상관관계 분석 시각화 중...")
             correlations = compute_correlation(drl_weights, reference_weights)
             ...
             print("통합 그래디언트 분석 중...")
             # 전체 데이터에서 일정 간격으로 샘플링
             sample_indices = np.linspace(0, len(test_data_aligned)-1, 5, dtype=int)
             for i, idx in enumerate(sample_indices):
                 state = test_data_aligned[idx]
                 ig_values = integrated_gradients(ppo_agent.policy, state)
                 if ig_values is not None:
                     plot_integrated_gradients(
                         ig_values.mean(axis=0) if ig_values.ndim > 1 else ig_values,
                         feature_names,
                         filename=f'integrated_gradients_sample{i}_{current_time}.png'
                     )
             
             print("설명 가능한 AI 분석 완료!")
             
         except Exception as e:
             print(f"설명 가능한 AI 분석 중 오류 발생: {str(e)}")
             import traceback
             traceback.print_exc()
