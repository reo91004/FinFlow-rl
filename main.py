...
def main():
    ...
    try:
        ...
        trainer = BIPDTrainer(
            train_data=train_data, test_data=test_data, save_dir=MODELS_DIR
        )
        ...
        training_results = trainer.train(
            n_episodes=N_EPISODES, save_interval=SAVE_INTERVAL
        )

        print("\n훈련 결과 요약:")
        print(f"  - 최종 평균 보상: {training_results['final_avg_reward']:.4f}")
        print(
            f"  - 최고 포트폴리오 가치: {training_results['best_portfolio_value']:,.0f}원"
        )
        print(f"  - 최고 샤프 비율: {training_results['best_sharpe_ratio']:.3f}")
        print(
            f"  - 훈련 안정성 (표준편차): {training_results['training_stability']:.4f}"
        )
        print()
        ...
        print("\n벤치마크 비교 결과:")
        print(
            f"  - 수익률 개선: {benchmark_results['outperformance']['value_improvement']:.2%}"
        )
        print(
            f"  - 샤프비율 개선: {benchmark_results['outperformance']['sharpe_improvement']:.3f}"
        )
        print(
            f"  - 최대낙폭 개선: {benchmark_results['outperformance']['drawdown_improvement']:.2%}"
        )
        print()
        ...
        print("\nBIPD 시스템 의사결정 설명:")
        print(
            f"  - T-Cell 위기 감지: {explanation['crisis_detection'].get('crisis_level', 0):.3f}"
        )
        print(
            f"  - 선택된 전략: {explanation['strategy_selection']['selected_strategy']}"
        )
        print(
            f"  - 메모리 시스템: {explanation['memory_system']['memory_count']}개 경험 보유"
        )

        specialization_scores = explanation["strategy_selection"][
        ...
        if success_rate > 0.7 and avg_improvement > 0.05:
            ...
            print(
                "❌ 개선 필요: 시스템 성능이 기대에 못 미칩니다. 하이퍼파라미터 조정이 필요합니다."
            )
