...
class StockPortfolioEnv(gym.Env):
    ...
    def _calculate_sharpe_ratio(self, window_size=REWARD_SHARPE_WINDOW):
        ...
        # 최근 수익률만 사용
        recent_returns = self.returns_history[-window_size:] if len(self.returns_history) >= window_size else self.returns_history
    ...
    def _calculate_volatility_scaling(self, window_size=20):
        ...
        # 최근 변동성 데이터만 사용
        recent_volatility = self.market_volatility_window[-window_size:] if len(self.market_volatility_window) >= window_size else self.market_volatility_window
        ...
        scaling = np.clip(avg_volatility / 0.01, REWARD_VOL_SCALE_MIN, REWARD_VOL_SCALE_MAX)
...
class PPO:
    ...
    def __init__(self, n_assets, n_features,
        ...
        if self.use_lr_scheduler:
            ...
            self.logger.info(f"Cosine Annealing LR 스케줄러 설정: T_max={LR_SCHEDULER_T_MAX}, eta_min={LR_SCHEDULER_ETA_MIN}")
    ...
    def check_early_stopping(self, validation_reward):
        ...
        if validation_reward > self.best_validation_reward:
            ...
            # 로깅
            self.logger.info(f"최고 검증 보상 {self.best_validation_reward:.4f} 대비 향상 없음. "
            ...
            if self.no_improvement_episodes >= self.early_stopping_patience:
                self.logger.warning(f"Early Stopping 조건 충족! {self.early_stopping_patience} 에피소드 동안 "
...
def apply_lime_explanation(model, state, n_samples=1000, kernel_width=0.25, feature_names=FEATURE_NAMES):
    """
    LIME(Local Interpretable Model-agnostic Explanations) 알고리즘을 적용하여 
    모델의 특정 의사결정에 대한 로컬 설명을 생성합니다.
    
    Args:
        model (ActorCritic): 설명할 ActorCritic 모델
        state (np.ndarray): 설명할 특정 상태 (n_assets, n_features)
        n_samples (int): 생성할 샘플 수
        kernel_width (float): 지역적 가중치 계산에 사용할 커널 너비
        feature_names (list): 특성 이름 목록
        
    Returns:
        dict: 각 자산과 특성별 중요도 점수
    """
    logger = logging.getLogger('PortfolioRL')
    ...
    try:
        ...
        with torch.no_grad():
            ...
            original_action = action_probs.squeeze(0).cpu().numpy()
        ...
        for _ in range(n_samples):
            # 0-1 사이의 랜덤 마스크 생성 (1: 유지, 0: 제거)
            # 각 특성별로 무작위로 활성화/비활성화 
            binary_mask = np.random.binomial(1, 0.5, size=(n_assets, n_features))
            
            # 원본 상태에서 마스크 적용 (주요 변동이 큰 가격/거래량 관련 특성은 적절히 스케일링)
            # 마스크가 0인 위치는 평균이나 다른 기준값으로 대체
            perturbed_sample = state.copy()
            for a in range(n_assets):
                for f in range(n_features):
                    if binary_mask[a, f] == 0:
                        # 특성이 가격 관련인 경우 (0-4번 인덱스)
                        if f < 5:
                            # 평균값 사용
                            perturbed_sample[a, f] = np.mean(state[:, f])
                        else:
                            # 기술적 지표는 스케일이 작으므로 0으로 대체해도 무방
                            perturbed_sample[a, f] = 0.0
        ...
        for asset_idx in assets_to_analyze:  # 선택된 자산만 분석
            ...
            if asset_idx < n_assets:
                ...
                feature_importance = np.zeros((n_assets, n_features))
                
                # 각 특성별 선형 회귀 학습
                for a in range(n_assets):
                    for f in range(n_features):
                        # 해당 특성의 마스크 수집
                        feature_masks = np.array([mask[a, f] for mask, _ in samples])
                        
                        # 해당 자산에 대한 예측값 수집
                        target_weights = np.array([pred[asset_idx] for pred in predictions])
                        
                        # 가중 선형 회귀 (kernel_weights 사용)
                        X = feature_masks.reshape(-1, 1)
                        coef = 0.0
                        
                        try:
                            # 가중 선형 회귀 (sklearn 안 사용하고 직접 구현)
                            X_weighted = X * kernel_weights.reshape(-1, 1)
                            y_weighted = target_weights * kernel_weights
                            
                            # X^T X 계산
                            xtx = np.dot(X_weighted.T, X)
                            if xtx > 1e-10:  # 행렬이 특이하지 않은 경우에만
                                # 계수 계산
                                xty = np.dot(X_weighted.T, y_weighted)
                                coef = xty / xtx
                            
                        except Exception as reg_err:
                            logger.warning(f"LIME 회귀 오류 (자산 {asset_idx}, 특성 {a},{f}): {reg_err}")
                        
                        # 계수 저장 (특성 중요도)
                        feature_importance[a, f] = coef
                
                # 자산 설명 저장
                explanations[asset_name] = feature_importance
            else:
                # 현금 자산 (n_assets 인덱스)
                asset_name = "Cash"
                cash_importance = np.zeros((n_assets, n_features))
                
                # 각 원본 자산 특성의 현금 비중에 대한 영향 계산
                for a in range(n_assets):
                    for f in range(n_features):
                        # 특성 마스크 수집
                        feature_masks = np.array([mask[a, f] for mask, _ in samples])
                        ...
                        try:
                            ...
                            logger.warning(f"LIME 현금 회귀 오류 (특성 {a},{f}): {reg_err}")
                        
                        cash_importance[a, f] = coef
                
                explanations[asset_name] = cash_importance
        
        return explanations
        
    except Exception as e:
        logger.error(f"LIME 설명 생성 중 오류: {e}")
        logger.error(traceback.format_exc())
        return {}

def plot_lime_explanation(lime_explanation, feature_names=FEATURE_NAMES, plot_dir=None, filename=None):
    """
    LIME 설명을 히트맵으로 시각화합니다.
    
    Args:
        lime_explanation (dict): apply_lime_explanation의 결과
        feature_names (list): 특성 이름 목록
        plot_dir (str): 저장할 디렉토리 경로
        filename (str): 저장할 파일 이름
    """
    logger = logging.getLogger('PortfolioRL')
    
    if not lime_explanation:
        logger.warning("LIME 시각화 실패: 설명 데이터가 비어있음")
        return
...
def train_ppo_agent(env: StockPortfolioEnv, ppo_agent: PPO, max_episodes: int, max_timesteps: int,
                      update_timestep: int, logger: logging.Logger, validation_env=None):
    """
    ...
    if ppo_agent.use_early_stopping:
        logger.info(f"Early Stopping 활성화: 인내심 {ppo_agent.early_stopping_patience} 에피소드, "
    ...
    for episode in pbar:
        ...
        while not terminated and not truncated:
            ...
            if total_steps % update_timestep == 0 and len(memory.states) >= ppo_agent.k_epochs:
                ...
                if ppo_agent.use_lr_scheduler:
                    ...
                    logger.debug(f" 정책 업데이트 {update_count}, 현재 학습률: {current_lr:.7f}")
                else:
        ...
        if ppo_agent.use_early_stopping and (episode + 1) % VALIDATION_INTERVAL == 0:
            ...
            logger.info(f"검증 에피소드 {episode+1}: 평균 보상 {validation_reward:.4f} (최고: {ppo_agent.best_validation_reward:.4f})")
            
            # Early Stopping 조건 확인
            should_stop = ppo_agent.check_early_stopping(validation_reward)
            if should_stop:
                logger.info(f"Early Stopping으로 학습 중단 (에피소드 {episode+1}/{max_episodes})")
                break
...
def fetch_benchmark_data(benchmark_tickers, start_date, end_date, save_path=DATA_SAVE_PATH):
    """
    ...
    os.makedirs(save_path, exist_ok=True)
    cache_file = os.path.join(save_path, f'benchmark_data_{"-".join(benchmark_tickers)}_{start_date}_{end_date}.pkl')
    ...
    try:
        # 진행 표시줄 없이 데이터 다운로드
        raw_data = yf.download(benchmark_tickers, start=start_date, end=end_date, progress=False)
            ...
            try:
                ...
                if isinstance(raw_data.columns, pd.MultiIndex):
                    ...
                    # 'Close'가 있는지 확인 (대체 방법)
                        ...
                        # 첫 번째 가격 관련 컬럼 사용
                        price_cols = [col for col in raw_data.columns.get_level_values(0) if col in ['Open', 'High', 'Low', 'Close']]
                        if price_cols:
                            ticker_data = raw_data[(price_cols[0], ticker)]
                            logger.warning(f"{ticker}: 'Adj Close'/'Close' 없음, '{price_cols[0]}' 사용")
                        else:
                ...
                # 단일 티커를 받아온 경우
                    ...
                    # 'Close'가 있는지 확인 (대체 방법)
                        ...
                        # 첫 번째 가격 관련 컬럼 사용
                        price_cols = [col for col in raw_data.columns if col in ['Open', 'High', 'Low', 'Close']]
                        if price_cols:
        ...
        logger.info(f"벤치마크 데이터 처리 완료 및 캐시 저장 (총 {len(benchmark_data)} 종목)")
        return benchmark_data
...
def calculate_benchmark_performance(benchmark_data, test_dates):
    ...
    for ticker, data in benchmark_data.items():
        ...
        benchmark_performance[ticker] = {
            'values': normalized_values.values * INITIAL_CASH,  # 포트폴리오와 동일한 초기 금액으로 스케일링
...
def plot_performance_comparison(portfolio_values, benchmark_performance, plot_dir, dates=None, title="Portfolio vs Benchmark", filename=None):
    """
    ...
    for i, (ticker, data) in enumerate(benchmark_performance.items()):
        ...
        if len(benchmark_values) > len(portfolio_values):
            ...
            # 부족한 길이는 마지막 값으로 채우기
            padding = np.full(len(portfolio_values) - len(benchmark_values), benchmark_values[-1])
            benchmark_values = np.concatenate([benchmark_values, padding])
            
        plt.plot(x_axis, benchmark_values, label=f'{ticker} Benchmark', 
                    linestyle='--', alpha=0.8, linewidth=1.5, color=colors[i % len(colors)])
    ...
    # 포트폴리오 누적 수익률 계산
    portfolio_returns = np.array([(portfolio_values[i] / portfolio_values[i-1]) - 1 
                                    for i in range(1, len(portfolio_values))])
    portfolio_cumulative_returns = (np.cumprod(1 + portfolio_returns) - 1) * 100  # 퍼센트로 변환
    
    # 포트폴리오 누적 수익률 플롯
    plt.plot(x_axis[1:], portfolio_cumulative_returns, label='PPO Portfolio', linewidth=2, color='blue')
    ...
    for i, (ticker, data) in enumerate(benchmark_performance.items()):
        ...
        benchmark_cumulative_returns = (np.cumprod(1 + benchmark_returns) - 1) * 100  # 퍼센트로 변환
        
        plt.plot(x_axis[1:], benchmark_cumulative_returns, label=f'{ticker} Benchmark', 
                    linestyle='--', alpha=0.8, linewidth=1.5, color=colors[i % len(colors)])
    ...
    # 파일 저장
    if filename is None:
        filename = f'portfolio_vs_benchmark_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png'
    save_path = os.path.join(plot_dir, filename)
...
def print_performance_comparison(portfolio_metrics, benchmark_performance, logger):
    ...
    for ticker, data in benchmark_performance.items():
        ...
        rel_annual_return = portfolio_metrics['annual_return'] - metrics['annual_return']
...
def main():
    ...
    if USE_BENCHMARK:
        ...
        benchmark_data = fetch_benchmark_data(BENCHMARK_TICKERS, TEST_START_DATE, TEST_END_DATE)
    else:
    ...
    validation_env = StockPortfolioEnv(train_data[-500:], normalize_states=False)  # 학습 데이터 마지막 500일 사용
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        training_rewards = train_ppo_agent(
            train_env, ppo_agent, max_episodes_train, max_timesteps_train, 
            PPO_UPDATE_TIMESTEP, logger, validation_env=validation_env
        )
    ...
    if USE_BENCHMARK and benchmark_data:
        logger.info("\n--- 벤치마크 성능 분석 ---")
        # 벤치마크 성능 계산
        benchmark_performance = calculate_benchmark_performance(benchmark_data, test_dates)
    ...
    # 4. LIME 분석 추가
    logger.info("LIME 분석 중...")
    lime_explanations = []
    
    # 테스트 샘플 중 첫 두 개만 사용 (계산 비용 절감)
    lime_samples = test_samples[:min(2, len(test_samples))]
    ...
    if lime_explanations:
        logger.info("\nLIME 분석 결과 요약:")
        # 첫 번째 설명 결과만 요약
        first_explanation = lime_explanations[0]
        for asset_name, importance in first_explanation.items():
            if importance.ndim == 2:
                # 2D 중요도 (자산, 특성) -> 특성별 평균
                avg_importance = np.mean(np.abs(importance), axis=0)
                logger.info(f"{asset_name} 상위 3개 중요 특성:")
                # 상위 3개 특성 찾기
                top_indices = np.argsort(avg_importance)[-3:][::-1]
                for idx in top_indices:
                    if idx < len(FEATURE_NAMES):
                        logger.info(f"  {FEATURE_NAMES[idx]}: {avg_importance[idx]:.4f}")
    else:
        logger.warning("LIME 분석 결과가 없습니다.")
