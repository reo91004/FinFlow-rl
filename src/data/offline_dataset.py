...
전략:
- random, momentum, mean_reversion
- equal_weight, risk_parity, min_variance
보상 스케일링: 100배 증폭 후 [-5, 5] 클리핑
데이터 증강: 노이즈 추가로 다양성 확보
"""
...
class OfflineDataset:
        ...
        if data_path:
            ...
            if data_path.is_dir():
                ...
                if npz_file.exists():
                    ...
                    self.logger.info(f"데이터 파일이 없음: {npz_file}")
            elif str(data_path).endswith('.npz'):
                # NPZ 파일 직접 지정
                if data_path.exists():
                    self._load_npz(data_path)
                else:
                    self.logger.info(f"데이터 파일이 없음: {data_path}")
            else:
                self.logger.info(f"지원하지 않는 형식: {data_path}")
    ...
    def _load_npz(self, file_path: Path):
        ...
        self.logger.info(f"NPZ 데이터셋 로드 완료: {file_path}")
        self.logger.info(f"  - 샘플 수: {self.size}")
    ...
    def get_all_data(self) -> Dict[str, np.ndarray]:
        """
        전체 데이터를 Dict로 반환 (OfflineTrainer 호환)
    ...
    def save(self, path: str):
        ...
        self.logger.info(f"데이터셋 저장: {path}")
