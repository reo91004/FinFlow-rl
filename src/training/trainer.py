...
"""
통합 학습 파이프라인: 오프라인 + 온라인 학습 관리

목적: 전체 학습 파이프라인 조정 (오프라인→온라인)
의존: algorithms/, environments/, data/, evaluation/ 모듈들
사용처: main.py, scripts/train.py
역할: Phase 1 오프라인 사전학습 + Phase 2 온라인 미세조정

파이프라인:
- Phase 1: OfflineDataset → OfflineTrainer → IQL/TD3BC
- Phase 2: BCell + TCell + Memory → REDQ 온라인 학습
- Phase 3: 평가 및 결과 저장

구현 내용:
- OfflineTrainer: IQL/TD3BC 오프라인 학습기
- FinFlowTrainer: 전체 파이프라인 관리자
- 절제 연구(ablation) 지원
"""
...
# 새로운 경로에 맞춰 import 수정
from src.environments.portfolio_env import PortfolioEnv
from src.environments.reward_functions import PortfolioObjective
...
from src.algorithms.offline.td3bc import TD3BCAgent
from src.evaluation.monitor import PerformanceMonitor
    ...
    def _initialize_components(self):
        ...
        if self.ablation_config['use_xai']:
            self.explainer = XAIExplainer(
                model=self.b_cell,
                feature_names=None  # FeatureExtractor에서 자동 생성
            )
        else:
            self.explainer = None
